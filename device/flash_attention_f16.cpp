...
#define ATTENTION_PADDED_VALUE_DIMENSION ATTENTION_PADDED_HEAD_DIMENSION
#endif
...
static_assert(ATTENTION_PADDED_VALUE_DIMENSION % 16 == 0);
...
__attribute__((always_inline)) bool mergeBlock(const BlockValues &blockValues,
                                               ...
                                               bool finalBlock,
                                               unsigned worker) {
  constexpr unsigned dimension = ATTENTION_VALUE_DIMENSION;
  ...
  for (unsigned row = worker; row < QueryRows; row += 6) {
    float *output = &accumulator[row * ATTENTION_PADDED_VALUE_DIMENSION];
    float &maximumState = output[dimension];
    float &denominatorState = output[dimension + 1];
    const float blockMaximum = blockMaxima[row];
    const float blockDenominator = blockDenominators[row];
    float previousScale = 0.0f;
    float blockScale = 1.0f;
    if (initialBlock) {
      maximumState = blockMaximum;
      denominatorState = blockDenominator;
    } else {
      const float maximum = __builtin_fmaxf(maximumState, blockMaximum);
      previousScale = __builtin_expf(maximumState - maximum);
      blockScale = __builtin_expf(blockMaximum - maximum);
      denominatorState = denominatorState * previousScale +
                         blockDenominator * blockScale;
      maximumState = maximum;
    }

    for (unsigned panel = 0; panel < ATTENTION_PADDED_VALUE_DIMENSION / 16;
         ++panel) {
#pragma unroll
      for (unsigned physicalPair = 0; physicalPair < 8; ++physicalPair) {
        const unsigned column = panel * 16 + physicalPair * 2;
    ...
    if (finalBlock) {
      const float reciprocal = 1.0f / denominatorState;
  ...
  public:                                                                      \
    ...
    bool compute(unsigned worker) {                                             \
      return mergeBlock<QueryRows>(blockValues, blockState, accumulator,        \
                                   initialBlock != 0, finalBlock != 0, worker);  \
    }                                                                          \
  }

DEFINE_MERGE_VERTEX(AttentionMergeSmallQueryF16, ATTENTION_SMALL_QUERY_ROWS);
DEFINE_MERGE_VERTEX(AttentionMergeLargeQueryF16, ATTENTION_LARGE_QUERY_ROWS);
