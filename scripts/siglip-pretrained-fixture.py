...
IMAGES = ['authors.jpg', 'siglip.jpg', 'caffeine.jpg', 'robosign.jpg', 'fried_fish.jpeg', 'cow_beach2.jpg']
...
def main():
    ...
    with safe_open(checkpoint, framework='pt') as f:
        state = {k: f.get_tensor(k) for k in f.keys() if k.startswith('vision_model.')}
    config = SiglipVisionConfig.from_pretrained(MODEL, revision=REVISION)
    config._attn_implementation = 'sdpa'
    with torch.device('meta'):
        model = SiglipVisionModel(config)
    model.load_state_dict({k.removeprefix("vision_model."): v for k, v in state.items()}, strict=True, assign=True)
    ...
    manifest = {'model': MODEL, 'revision': REVISION, 'parameters': {}, 'cases': [],
                'config': config.to_dict(), 'preprocessor': processor.to_dict()}

    def write(path, tensor):
        array = tensor.detach().cpu().float().contiguous().numpy().astype('<f4')
        array.tofile(root / path)
        return {'file': str(path), 'shape': list(array.shape),
                'sha256': hashlib.sha256(array.tobytes()).hexdigest()}

    def put(name, tensor):
        manifest['parameters']['vit.' + name] = write(Path('parameters') / (name + '.f32'), tensor)
    ...
    put('embedding.weight', get('embeddings.patch_embedding.weight').permute(2, 3, 1, 0).reshape(588, 1152))
    ...
    for i in range(27):
        ...
        for suffix in ('weight', 'bias'):
            fused = torch.cat([get(src + f'.self_attn.{q}_proj.{suffix}') for q in 'qkv'])
            put(dst + '.attention.qkv.' + suffix, fused.T if suffix == 'weight' else fused.reshape(1, 1, -1))
    ...
    for suffix in ('weight', 'bias'):
        ...
        for name, value in [('query', q), ('kv', torch.cat([k, v]))]:
            put('map.attention.' + name + '.' + suffix, value.T if suffix == 'weight' else value.reshape(1, 1, -1))
    ...
    for filename in IMAGES:
        ...
        pixels = processor(images=Image.open(original).convert('RGB'), return_tensors='pt').pixel_values
        ...
        patches = pixels[:, :, :378, :378].reshape(1, 3, 27, 14, 27, 14).permute(0, 2, 4, 3, 5, 1).reshape(1, 729, 588)
        case = {'name': directory.name, 'url': url,
                'image_sha256': hashlib.sha256(original.read_bytes()).hexdigest(),
                'inputs': {'vit.image.patches': write(Path(directory.name) / 'patches.f32', patches)},
                'expected': write(Path(directory.name) / 'expected.f32', expected)}
        manifest['cases'].append(case)
        print(f'{filename}: FP32 reference norm={expected.norm().item():.6f}', flush=True)
