...
pub mod application_capnp {
    pub use crate::profile_capnp::{
        ExchangeActivityKind as ProfileExchangeActivityKind, StepKind as ProfileStepKind,
        exchange_activity as profile_exchange_activity, metadata_entry as profile_metadata,
        profile_step as profile_step_plan,
    };
...
fn write_profile_tiles(
    ...
    for (tile_index, tile) in tiles.iter().enumerate() {
        ...
        for (step_index, step) in tile.steps.iter().enumerate() {
            step.write(steps.reborrow().get(step_index as u32));
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn reads_legacy_package_profile_records() {
        // Encoded with the pre-shared v5 application schema, not the current writer.
        let bytes = include_bytes!("../tests/fixtures/profile-schema-v5.ipuexe");
        let app = Application::read(bytes.as_slice()).unwrap();
        let steps = &app.profile_tiles[0].steps;
        assert_eq!(
        ...
        assert_eq!(
            ...
            ProfileStep {
                ...
                phase: 7,
                epoch: 3,
                operation: "operation.4".into(),
                kind: ProfileStepKind::Exchange,
                kernel: "multicast".into(),
                metadata: vec![ProfileMetadata {
                    name: "reason".into(),
                    value: "LayoutRearrangement".into()
                }],
                exchange_activities: [
                    (ProfileExchangeActivityKind::Send, 2, 8, 3, true),
                    (ProfileExchangeActivityKind::Receive, 4, 10, 2, false),
                    (ProfileExchangeActivityKind::PartnerBusy, 1, 12, 3, true),
                ]
                .into_iter()
                .map(
                    |(kind, start_cycle, end_cycle, fanout, paired)| ProfileExchangeActivity {
                        kind,
                        start_cycle,
                        end_cycle,
                        fanout,
        ...
        assert_eq!(Application::read(encoded.as_slice()).unwrap(), app);
    }
