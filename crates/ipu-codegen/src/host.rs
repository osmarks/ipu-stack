...
pub(crate) fn plan(
    ...
    for transfer in output_phases.iter().flat_map(|phase| &phase.transfers) {
        ipu_target::exchange::plan_tile_to_host(
            transfer.physical_tile,
            transfer.tile_address,
            transfer.host_offset,
            transfer.bytes,
...
fn phase_instructions(
    ...
    Ok(match (target, xreq) {
        ...
        (None, Some(xreq)) => (
            ipu_target::exchange::wrap_host_xreq_operation(physical_tile, &xreq.instructions)?,
            xreq.packet_words,
        ),
        (Some(target), None) => (
            ipu_target::exchange::wrap_host_target_operation(physical_tile, &target.instructions)?,
            target.packet_words,
        ),
...
fn target_program(
    ...
    Ok(match transfer.direction {
        ...
        Direction::ToHost => ipu_target::exchange::assemble_tile_to_host_target_program(
            transfer.physical_tile,
            transfer.tile_address,
            transfer.host_offset,
            transfer.bytes,
...
fn xreq_targets(physical_tile: u16, phase: &Phase) -> PackageBuildResult<Vec<u16>> {
    phase
        ...
        .iter()
        .filter_map(
            |transfer| match ipu_target::exchange::host_hierarchy(transfer.physical_tile) {
