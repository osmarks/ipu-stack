...
pub fn diagnose_exchange_tile(
    ...
    Ok(ExchangeTileDiagnostic {
        ...
        row_address,
        row_elements,
        program: ipu_target::exchange::parse::diagnose_plan_program(program, Some(row_address))?,
        activities,
    })
}
...
fn lower_static_exchanges(
    ...
    program
        ...
        .map(|phase| {
            ...
            // Paired-width transfers remain available to the standalone
            // exchange diagnostics, but are not selected by package
            // lowering. Their previous per-transfer search rebuilt the
            // complete phase for every candidate and did not produce a
            // worthwhile runtime improvement.
            let ScheduledPending {
                pending,
                incoming_bases,
                optimized,
            } = optimize_owned_pending(topology, pending, program.tile_count)?;
...
fn materialize_schedule_order(
    ...
    if schedule_encoding_is_valid(&schedule)? {
        ...
        Err(ipu_target::exchange::ExchangeError::Schedule("SENDPICP instruction alignment").into())
    }
}
