...
pub use exchange::{
    ...
    PhysicalExchangePhase, inactive_exchange_row, lower_exchanges,
};
pub use graph::{
...
pub struct ExchangeStep {
    ...
    pub row: Vec<u32>,
    /// Immutable alternate rows used by an enclosing structured repeat.
    #[serde(default)]
    pub repeat_variants: Vec<PlacedExchangeRow>,
    #[serde(default)]
    pub profile: StepProfile,
}
        ...
        match step {
            TileStep::Exchange(exchange) => {
                ...
                if exchange.repeat_variants.is_empty() {
                    emit_static_exchange_call(code, exchange, worker_barrier)?;
                } else {
                    emit_repeat_exchange_call(code, exchange, worker_barrier, code_address)?;
                }
                ...
                exchange_rows.extend(exchange.repeat_variants.iter().cloned());
            }
    ...
    for step in steps {
        match step {
            TileStep::Exchange(exchange) => {
                ...
                validate_exchange_row(&exchange.row)?;
            }
            TileStep::Compute(compute) => {
...
fn validate_exchange_row(row: &[u32]) -> Result<()> {
    let inactive = row.first() == Some(&SANS_INACTIVE_INSTRUCTION);
    if row.len() < 3
        || (!inactive && row.first() != Some(&SYNC_SUPERVISOR_INSTRUCTION))
        || !row.contains(&ipu_exchange::RETURN_M10_INSTRUCTION)
    {
        return Err(invalid(
...
fn active_exchange(step: &TileStep) -> bool {
    match step {
        TileStep::Exchange(exchange) => {
            exchange.row.first() != Some(&SANS_INACTIVE_INSTRUCTION)
                || exchange
                    .repeat_variants
                    .iter()
                    .any(|variant| variant.words.first() != Some(&SANS_INACTIVE_INSTRUCTION))
        }
...
fn emit_static_exchange_call(
    code: &mut TileCode,
    exchange: &ExchangeStep,
    worker_barrier: Option<u32>,
) -> Result<()> {
    if exchange.row[0] != SANS_INACTIVE_INSTRUCTION {
        code.call(worker_barrier.expect("active exchange has barrier"), 7)?;
    }
    code.call(exchange.address, 10)
}

fn emit_repeat_exchange_call(
    code: &mut TileCode,
    exchange: &ExchangeStep,
    worker_barrier: Option<u32>,
    code_address: u32,
) -> Result<()> {
    let variants = std::iter::once(PlacedExchangeRow {
        address: exchange.address,
        words: exchange.row.clone(),
    })
    .chain(exchange.repeat_variants.iter().cloned())
    .collect::<Vec<_>>();
    code.ld32(0, 11, 15, 0)?;
    let mut selections = Vec::with_capacity(variants.len() - 1);
    for variant in (1..variants.len()).rev() {
        code.add_immediate(0, 0, -1)?;
        let branch = code.words.len();
        code.brz(0, 0)?;
        selections.push((branch, variant));
    }
    let mut exits = Vec::with_capacity(variants.len());
    emit_static_exchange_call(
        code,
        &ExchangeStep {
            address: variants[0].address,
            row: variants[0].words.clone(),
            repeat_variants: Vec::new(),
            profile: StepProfile::default(),
        },
        worker_barrier,
    )?;
    exits.push(code.words.len());
    code.jump(0)?;
    for (branch, variant) in selections {
        let target = code.address(code_address)?;
        code.words[branch] = encode_brz_m_immediate(0, target)?;
        emit_static_exchange_call(
            code,
            &ExchangeStep {
                address: variants[variant].address,
                row: variants[variant].words.clone(),
                repeat_variants: Vec::new(),
                profile: StepProfile::default(),
            },
            worker_barrier,
    ...
    let continuation = code.address(code_address)?;
    for exit in exits {
        code.words[exit] = encode_setzi_m(0, continuation)?;
    }
    Ok(())
}
...
fn emit_repeat(
    ...
    if repeat.iterated_pointers.iter().any(|pointer| {
        pointer.addresses.len() != repeat.count as usize || pointer.addresses.is_empty()
    }) {
        return Err(invalid("repeat pointer table has invalid shape"));
    }
    let words = repeat
        .iterated_pointers
        .len()
    ...
    for (index, pointer) in repeat.iterated_pointers.iter().enumerate() {
        code.setzi(0, pointer.addresses[0])?;
        code.st32(
            0,
            11,
            15,
    ...
    let done_branch = code.words.len();
    code.brz(0, 0)?;
    emit_repeat_pointer_selection(code, repeat, code_address)?;
...
fn emit_repeat_pointer_selection(
    code: &mut TileCode,
    repeat: &RepeatStep,
    code_address: u32,
) -> Result<()> {
    let mut selections = Vec::with_capacity(repeat.count.saturating_sub(1) as usize);
    for iteration in (1..repeat.count as usize).rev() {
        code.add_immediate(0, 0, -1)?;
        let branch = code.words.len();
        code.brz(0, 0)?;
        selections.push((branch, iteration));
    }
    let mut exits = Vec::with_capacity(selections.len() + 1);
    for (branch, iteration) in selections {
        let target = code.address(code_address)?;
        code.words[branch] = encode_brz_m_immediate(0, target)?;
        for (index, pointer) in repeat.iterated_pointers.iter().enumerate() {
            code.setzi(0, pointer.addresses[iteration])?;
            code.st32(
                0,
                11,
                15,
