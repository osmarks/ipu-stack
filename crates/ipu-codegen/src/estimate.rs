...
pub(crate) fn conversion_exchange_bytes_per_transfer(
    input: &TensorType,
    output: &TensorType,
) -> u64 {
    ...
    if input.format.layout.tiling.tile_count != output.format.layout.tiling.tile_count {
        return u64::MAX;
    }
    ...
    if tiles > 8 {
        let elements = (0..output.shape.0.len()).try_fold(1u64, |elements, axis| {
            let mut input_ranges = (0..tiles)
                .filter_map(|tile| tile_axis_range(input, axis, tile))
                .collect::<Vec<_>>();
            let mut output_ranges = (0..tiles)
                .filter_map(|tile| tile_axis_range(output, axis, tile))
                .collect::<Vec<_>>();
            input_ranges.sort_by_key(|range| (range.start, range.end));
            output_ranges.sort_by_key(|range| (range.start, range.end));
            input_ranges.dedup();
            output_ranges.dedup();
            let mut maximum = 0u32;
            let (mut left, mut right) = (0, 0);
            while left < input_ranges.len() && right < output_ranges.len() {
                let input_range = &input_ranges[left];
                let output_range = &output_ranges[right];
                let start = input_range.start.max(output_range.start);
                let end = input_range
                    .end
                    .min(output_range.end)
                    .min(output.shape.0[axis]);
                maximum = maximum.max(end.saturating_sub(start));
                if input_range.end <= output_range.end {
                    left += 1;
                } else {
                    right += 1;
                }
            }
            (maximum != 0).then_some(elements.saturating_mul(u64::from(maximum)))
        });
        return elements
            .unwrap_or(0)
            .saturating_mul(output.format.precision.bytes());
    }
    (0..tiles).fold(0u64, |maximum, destination| {
        (0..tiles)
            .filter(|&source| source != destination)
            .fold(maximum, |maximum, source| {
                let elements = (0..output.shape.0.len()).try_fold(1u64, |elements, axis| {
                    let input_range = tile_axis_range(input, axis, source)?;
                    let output_range = tile_axis_range(output, axis, destination)?;
                    let start = input_range.start.max(output_range.start);
                    let end = input_range
                        .end
                        .min(output_range.end)
                        .min(output.shape.0[axis]);
                    (start < end)
                        .then_some(elements.saturating_mul(u64::from(end.saturating_sub(start))))
                });
                maximum.max(
                    elements
                        .unwrap_or(0)
