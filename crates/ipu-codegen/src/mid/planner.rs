            ...
            let (estimated_cycles, estimated_exchange_cycles, peak_memory) =
                if let Some(Some((program, peak, true))) = branch.analysis.get() {
                    ...
                    (
                        ...
                        region_peak_memory(config, &initial, &branch.operations, &outputs, &branch.state.values),
                    )
                };
            tracing::info!(
                finalist,
...
pub(super) fn lower_operation_candidates(
    ...
    for (operation_index, operation) in source.iter().enumerate() {
        ...
        let mut regions = match &operation.kind {
            OperationKind::Repeat(repeat) => Some(super::region::RegionSearch::new(
        ...
        for branch in beam {
            if let OperationKind::Repeat(repeat) = &operation.kind {
                ...
                let body_candidates = match lower_repeat_candidates(
                    ...
                    Err(
                        error @ (LoweringError::NoCandidate(_)
                        | LoweringError::InsufficientMemory { .. }),
                    ) => {
                        region_failure = Some(error);
                        continue;
                    }
                    Err(error) => return Err(error),
                };
                // Compare complete memory footprints before pruning the product
                // of outer prefixes and body alternatives. The body cache itself
                // deliberately does not depend on unrelated outer live values.
                expanded.extend(
                    body_candidates
                        .into_par_iter()
                        .map(|mut next| {
                    ...
                    .flat_map_iter(|plan| {
                        ...
                        let compact_inputs = input_ids
                            ...
                            .filter_map(|(&id, requirement)| {
                                ...
                                .then_some((id, requirement.format.layout.order))
                            })
                            .collect::<Vec<_>>();
                        let orders = orders
                            .into_iter()
                        ...
                        orders.into_iter().map(move |(cast_orders, compact)| {
                            ...
                            next.analysis.take();
                            if compact {
                                for &(id, order) in &compact_inputs {
                                    let value = next.state.get(id);
                                    let copies = constraints.allocation_copies[&value.origin];
                                    let mut storage = value.tensor_type.clone();
                                    storage.format.layout.order = order;
                                    let Some(layout) = super::ownership::compact_parameter_layout(
                                        &storage, copies, config,
                                    ) else {
                                        continue;
                                    };
                            ...
                            if compact {
                                let origins = compact_inputs
                                    .iter()
                                    .map(|(id, _)| next.state.get(*id).origin)
                                    .collect::<BTreeSet<_>>();
        ...
        if expanded.is_empty() {
            ...
            return Err(region_failure.unwrap_or(LoweringError::NoCandidate(operation.id)));
        }
...
fn lower_repeat_candidates(
    ...
    graph: &ComputeGraph,
    costs: &impl CostModel,
    search: &mut super::region::RegionSearch<'_, impl CostModel>,
) -> LoweringResult<Vec<BeamBranch>> {
    let mut prepared = branch.clone();
    prepared.analysis.take();
    let values = &prepared.values;
    let state = &mut prepared.state;
    let inputs = operation
        .inputs
        .iter()
    ...
    let bindings = inputs
        .iter()
        .copied()
        .chain(raw_iterated_inputs.iter().map(|sequence| sequence[0]))
        .collect::<Vec<_>>();
    let boundary = super::region::RegionBoundary::new(
        &repeat.body.arguments,
        &bindings,
        state,
        |index| {
            if index < inputs.len() {
                // Carried layouts must remain stable across iterations.
                index >= repeat.carried_inputs && state.automatic_inputs.contains(&inputs[index])
            } else {
                iterated_automatic[index - inputs.len()]
            }
        },
        |index| {
            if index < inputs.len() {
                state.parameter_values.contains(&inputs[index])
            } else {
                iterated_parameters[index - inputs.len()]
            }
        },
    );
