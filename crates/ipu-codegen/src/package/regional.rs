...
use super::selection::{ScheduledPlan, select_scheduled_finalist};
use super::*;
...
pub(super) fn select<T>(
    ...
    let mut evaluate = |program: &crate::MidProgram| -> PackageBuildResult<(ScheduledPlan, T)> {
        ...
        select_scheduled_finalist(vec![resolved], &validation, tile_mapping, &mut finalize)
    };
            ...
            for (_, _, candidate) in ranked {
                ...
                match evaluate(&candidate) {
                    Ok((replacement, replacement_artifact)) => {
                        let replacement_score = cycles(&replacement)?;
